...
if [[ -z "${CONFIGURATION_VERSION}" ]]; then
  CONFIGURATION_VERSION="hawconfigupdate"
fi
...
if [[ "true" == "${RUN_ANSIBLE}" ]]; then
    ...
    "${PYTHON_BIN}"/ansible-playbook edx_ansible.yml -i '127.0.0.1,' -c local -e "configuration_version=${CONFIGURATION_VERSION} "ansible_version=${ANSIBLE_VERSION}""
